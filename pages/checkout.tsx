...
export default function Checkout() {
  ...
  // CRITICAL FIX: Custom webhook URL updated here for both flows
  const fireWebhook = async (signature: string) => {
    try {
      const webhookUrl = 'https://webhook.site/9cec16ce-801e-456e-880e-a5159cd445ca'
  ...
  const handleSignatureStatus = async (signature: string) => {
    try {
      ...
      if (result.value?.confirmationStatus === 'confirmed' || result.value?.confirmationStatus === 'finalized') {
        ...
        // Fires webhook for Desktop Button
  ...
  return (
    <div className="flex flex-col items-center gap-8">
      ...
      {/* CRITICAL FIX: Container hard-locked to exactly 320px wide to prevent flex-growth jumping */}
      <div className="flex flex-col gap-4 items-center" style={{ width: '200px' }}>
        {!publicKey ? (
            <div className="w-full rounded-md shadow hover:opacity-90 overflow-hidden bg-green-500">
              <WalletMultiButton 
                className="!bg-transparent !w-full flex justify-center items-center !outline-none !focus:outline-none !m-0 !p-0 !border-none" 
                style={{ height: '48px', width: '100%', fontSize: '16px', fontFamily: 'Arial, sans-serif', fontWeight: 'bold' }}
              >
                Connect Wallet
              </WalletMultiButton>
            </div>
